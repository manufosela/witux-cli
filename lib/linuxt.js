...
function processArgs() {
  ...
  if (command === 'create-page') {
    if (argv._.length > 1) {
      pageName = argv._[1];
      languages = argv.languages || argv.m || '';  // es,en
  ...
  return new Promise( resolve => {
    ...
    for (const file of files) {
      ...
      if (extname === '.html') {
        ...
        filePromises.push(getPageContent(file, dir));
        console.log(`------------- ${file} saved into ${distPath}`);
      } else if (!notProcessPaths.includes(file)) {
        fs.lstat(file, async (err, stat) => {
          if (stat.isDirectory()) {
...
function processPath(dir = workdir, complementPath = '') {
  ...
  return new Promise(async resolve => {
    ...
    console.log(`Procesando path ${dir}...`);
    fs.mkdirSync(distPath);
    console.log(`Creado ${distPath}...`);
...
function createPage(pageDir = currentWorkDir){
  ...
  if (languages !== '') {
    const langArr = languages.split(',');
    langArr.forEach(lang => {
      fs.copyFileSync(path.join(origin, '_base.html'), path.join(pageDir, lang, pageName + '.html'));
      const pathFileName = path.join(pageDir, lang, pageName + '.html');
      ...
      content = content.replace(/<!-- __ALTERNATES__ -->/, _getAlternates(langArr, lang, pageName));
      fs.writeFileSync(pathFileName, content);
    });
    if (pageName === 'index') {
      filesToProcess.push({ 'filename': '_base.html', 'path': '.', 'output': pageName + '.html'  });
    }
  } else {
    filesToProcess.push({ 'filename': '_base.html', 'path': '.', 'output': pageName + '.html'  });
  }
