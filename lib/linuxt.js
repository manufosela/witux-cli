...
function createScafolding() {
  ...
  shell.mkdir(siteDir);
  const directories = [
    path.join(wcDir, 'resources'),
    path.join(wcDir, 'src'),
    path.join(wcDir, 'src', 'assets'),
    path.join(wcDir, 'src', 'assets', 'fonts'),
    path.join(wcDir, 'src', 'assets', 'images'),
    path.join(wcDir, 'src', 'components'),
    path.join(wcDir, 'src', 'css'),
    path.join(wcDir, 'src', 'js'),
    path.join(wcDir, 'src', 'js', 'lib'),
    path.join(wcDir, 'src', 'js', 'pages'),
    path.join(wcDir, 'src', 'js', 'tpl'),
    path.join(wcDir, 'src', 'json')
  ];
  for (const dir of directories) {
    createDir(dir);
  }
  ...
  for (const file of filesToProcess) {
    fs.copyFileSync(path.join(siteDir, file.path, file.filename), path.join(wcDir, file.path, file.output));
  }
}
