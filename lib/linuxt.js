...
function getPageContent(file, distPath = '') {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const url = `${serverURL}/${fixedDistPath}${file}`;
    console.log(url);
...
function getXmlPage(urlBase, pathPage, dateUpdate) {
  return `<url>
    <loc>${urlBase}/${pathPage}</loc>
    <lastmod>${dateUpdate}</lastmod>
    <changefreq>monthly</changefreq>
    <priority>0.5</priority>
  </url>`;
}

function generateSitemap(pathSitemap, urlBase, pages) {
  return new Promise((resolve) => {
    const today = new Date();
    const dateUpdate = `${today.getFullYear()}-${today.getFullYear.getMonth()}-${today.getDay()}`;
    const xmlPages = pages.filter((page) => {
      getXmlPage(urlBase, page, dataUpdate);
    });
    const baseSitemap = `
    ...
    `;
    fs.writeFileSync(pathSitemap, baseSitemap);
  });
}
