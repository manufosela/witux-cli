...
function prepareDist() {
  return new Promise(async resolve => {
    ...
    var distDirs = srcDirs.filter( dir => !excludeDirs.includes(dir));
...
function executeCommand(cmd, params) {
  const child = spawn(cmd, [params], { cwd: process.cwd(), detached: true, stdio: 'inherit' });
